...
def regresion_lineal(data, target):
    X = data.drop([target], axis=1)
    reg = RegresionLineal(X.columns, 'Precio')
    X_train, X_test, Y_train, Y_test = reg.train_test_split(data)
    reg.fit(X_train, Y_train)
    Y_pred = reg.predict(X_test)
    mse, r2 = reg.evaluate(Y_test, Y_pred)
    print(f'Mean Squared Error: {mse}')
    print(f'R2 Score: {r2}')
    reg.plot(Y_test, Y_pred)
